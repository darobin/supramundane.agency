...
import { collections, HOME_PAGE, titleOf, forEachAsset } from './schema.js';
...
export async function deleteItem(type, id) {
  ...
  const inUse = new Set();
  for (const s of siblings) forEachAsset(type, s.data, (v) => inUse.add(v));
  const gone = [];
  forEachAsset(type, item.data, (v) => { if (!inUse.has(v)) gone.push(v); });
  for (const v of gone) await rm(path.join(item.dir, v), { force: true });
  return true;
}
